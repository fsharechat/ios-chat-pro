...
- **Task 5's `recordIncomingMessage(incrementUnread:)` boolean flag is a forward-looking risk worth flagging for Plan D, not a Task 5 defect.** Code review confirmed the fetch-then-mutate-then-`.save()` pattern is genuinely race-free (verified directly against GRDB's source: `DatabaseQueue.write` serializes the entire closure, including the initial read, inside one transaction on one serial queue — not just assumed). The remaining risk is API-shape: `incrementUnread: Bool` has no compiler-enforced link to *why* it should be true or false (it should be `false` only when recording my own sent message), so a future Plan D call site could pass the wrong literal with only a doc comment as the safety net — the same class of "schema/business rule not propagating to call sites" issue Task 4 hit with `direction` scoping, just at the API-design level instead of the query level. When Plan D writes the real caller, prefer deriving this from the message's actual `direction` (or splitting into `recordReceivedMessage`/`recordSentMessage`) rather than threading a bare boolean through.
