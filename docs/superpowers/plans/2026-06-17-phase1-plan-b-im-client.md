...
- **Task 10 code-quality review found a real concurrency gap, fixed without touching internal locking or test determinism:** `IMClient` has no internal synchronization, while its production transport (`NWConnectionTransport`) originally delivered callbacks on its own private background queue and `DispatchQueueScheduler` defaults to `.main` — a real cross-queue race if a caller drives `IMClient` from main (the common case). Adding internal `queue.async` hopping inside `IMClient` was considered and rejected: it would make `IMClient` asynchronous relative to `ManualScheduler`/`FakeTransportConnection`'s deliberately-synchronous test doubles, breaking the "assert immediately after triggering an event" pattern all 8 `IMClientTests` rely on. Instead: `NWConnectionTransport` now defaults its callback queue to `.main` (matching `DispatchQueueScheduler`), and `IMClient`'s doc comment now states an explicit single-queue-caller contract (the same pattern `URLSession`'s default delegate queue uses) — mirroring how `HeartbeatManager`'s own thread-safety gap was handled in Task 4 (documented, not silently dropped). Also fixed in the same pass: a missing `userDisconnect` guard in `sendConnectMessage` (defense against a late `.connected` event reaching it after `disconnect()`), defensive nilling of `transport.onEvent`/`onDataReceived` before cancel, and a since-inaccurate doc-comment claim in Plan A's `FrameDecoder` that implied `IMClient` internally serializes access (it doesn't — it's a caller convention).
...
- **No logging facility exists anywhere in this package** (confirmed: zero `print`/`os_log`/`Logger` calls in `IMClient`/`IMTransport`/`IMProto`). This matters concretely in `ConnectAckHandler.handle(frame:)`, which silently drops a malformed `Im_ConnectAckPayload` (`try? ... else { return }`) with no way for a host app to ever learn that happened — and more broadly for diagnosing real-device connect/heartbeat/reconnect issues once Plan C/D ship an actual app. Flagged here explicitly (found missing from this self-review during the final whole-implementation review) as a gap Plan C or D should close, e.g. an injectable logging callback/closure threaded through `IMClient` and its handlers, before relying on this layer in the field.
