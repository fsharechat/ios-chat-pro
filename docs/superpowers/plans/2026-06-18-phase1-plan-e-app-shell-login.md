...
- **Task 11's `simctl install`/`launch`/screenshot steps could not be completed in this environment.** `swift test` (169/169) and `xcodebuild ... build` (`** BUILD SUCCEEDED **`) both ran cleanly, repeatedly. `xcrun simctl boot`/`list`/`shutdown` all responded normally (confirmed "iPhone 15" reaches the `Booted` state). But `xcrun simctl install` hung indefinitely (confirmed twice, including after a full CoreSimulatorService restart and a freshly-confirmed-booted device) — consistent with this environment lacking whatever Simulator.app-adjacent installation subsystem `simctl install` depends on, not a defect in this plan's code. The actual-device-screen verification this step was meant to provide (does the login screen visually render on a fresh install) is therefore **not done** and is a real gap versus the plan's original intent — flagged here rather than silently skipped. If a future session has a working `simctl install` path (e.g. via Xcode's GUI "Run" instead of the CLI), that's the way to close this gap; it doesn't need a code change.
