...
```
HTTP POST /send_code  {mobile}                  → 发送验证码
HTTP POST /login      {mobile, code, clientId}   → 返回 {token, uid}

token 解析(必须与服务端字节级对齐,Phase 1 第一个要验证的风险点):
  1. Base64 解码 token
  2. AES 解密(空密钥)→ 得到 JSON,取出 password、secret 字段
  3. 用 secret 作为 key,对 password 做 AES 加密 → Base64 编码 → 得到 TCP 连接密码

TCP  Signal.CONNECT     {userName: uid, password: 上一步算出的密码, clientIdentifier: 设备UUID}
...
1. **AES 握手细节** —— token 解密与 CONNECT 密码加密的具体 mode/padding 未在本次调研中完全确认字节级细节,需要在 Phase 1 实现阶段第一步对照 Android `AES.java` 源码逐字节验证;这是阻塞性风险,验证不通过则后续所有联调工作都无法进行
